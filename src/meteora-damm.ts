...
export async function buildCreatePoolTransaction(
  input: z.infer<typeof createPoolAndAddLiquidityInputSchema>
): Promise<{
  success: boolean;
  transaction: string | null; // Base64 encoded
  poolAddress: string | null;
  positionAddress: string | null;
  positionNftMint?: string; // NFT mint address for the position (for transfers)
...
export async function buildAddLiquidityTransaction(
  input: z.infer<typeof addLiquidityInputSchema>
): Promise<{
  success: boolean;
  transactions: string[]; // Array of base64 encoded transactions
  positionAddress: string | null;
  positionNftMint?: string; // NFT mint address for the position (for transfers)
  liquidityDelta?: string; // Liquidity amount for lock transaction
  error: string | null;
}> {
  try {
    const cpAmm = getCpAmm();
...
export async function buildLockPositionTransaction(
  input: z.infer<typeof lockPositionInputSchema>
): Promise<{
  success: boolean;
...
export async function buildLiquidityProvisioningTransactions(
  ...
  positionAddress: string | null;
  positionNftMint?: string; // NFT mint address for the position (for transfers)
  liquidityDelta?: string; // Liquidity amount for locking after position is created
  isNewPool: boolean;
  error: string | null;
