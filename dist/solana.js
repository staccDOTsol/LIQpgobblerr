...
import { z } from 'zod';
import { submitSequentialTransactions, buildCreateNonceAccountsTransaction, NONCE_RENT_LAMPORTS, } from './nonce.js';
// Helius RPC endpoint
 ...
 */
export async function getFeeSwapTransaction(inputMint, amount, userPublicKey) {
 ...
 */
export async function getJupiterSwapTransaction(quoteResponse, userPublicKey) {
    try {
 ...
 */
export async function buildCreateNonceAccounts(input) {
 ...
 */
export async function submitJitoBundle(input) {
 ...
 */
export async function checkMeteoraPoolExists(tokenA, tokenB) {
    try {
        // Query Meteora API for existing pools
        const response = await fetch(`https://dlmm-api.meteora.ag/pair/all`);
        const pools = await response.json();
        // Search for a pool with the token pair
        const pool = pools.find((p) => (p.mint_x === tokenA && p.mint_y === tokenB) ||
