import { eq, and, or, asc, isNull, lte, desc } from "drizzle-orm";
 ...
 */
export async function insertProcessedIncoming(record) {
    const db = await getDb();
    if (!db) {
 ...
 */
export async function updateProcessedIncoming(signature, updates) {
    const db = await getDb();
    if (!db) {
 ...
 */
export async function getProcessedIncoming(signature) {
    const db = await getDb();
    if (!db)
        return undefined;
    const result = await db.select().from(processedIncoming)
 ...
 */
export async function getProcessedIncomingBySignature(signature) {
    return getProcessedIncoming(signature);
}
/**
 * Mark a transaction for retry
 */
export async function markForRetry(signature, error) {
    const db = await getDb();
    if (!db)
        return;
 ...
 */
export async function getTransactionsForRetry(limit = 10) {
    const db = await getDb();
    if (!db)
        return [];
    const now = new Date();
    return db.select().from(processedIncoming)
        .where(and(eq(processedIncoming.status, 'retry'), or(isNull(processedIncoming.nextRetryAt), lte(processedIncoming.nextRetryAt, now))))
 ...
 */
export async function getRecentProcessedIncoming(limit = 50) {
    const db = await getDb();
    if (!db)
        return [];
    return db.select().from(processedIncoming)
