 ...
 */
export async function buildAtomicLPTransaction(poolAddress, tokenAMint, tokenBMint, tokenAAmount, tokenBAmount, systemWallet, recipientAddress, slippageBps = 5000) {
    try {
        ...
        const ownerNftAta = await getAssociatedTokenAddress(positionNftMintPubkey, user, false, TOKEN_2022_PROGRAM_ID);
        ...
        const recipientNftAta = await getAssociatedTokenAddress(positionNftMintPubkey, recipient, false, TOKEN_2022_PROGRAM_ID);
        console.log('Recipient NFT ATA:', recipientNftAta.toBase58());
        // Create recipient's ATA instruction
        const createRecipientAtaIx = createAssociatedTokenAccountInstruction(user, // payer
        recipientNftAta, recipient, positionNftMintPubkey, TOKEN_2022_PROGRAM_ID);
        ...
        const transferNftIx = createTransferInstruction(positionNftAccount, // source: PDA where NFT was minted
 ...
 */
export async function submitAtomicTransaction(serializedTx) {
    try {
 ...
 */
export async function buildAtomicPoolCreationTransaction(tokenAMint, tokenBMint, tokenAAmount, tokenBAmount, systemWallet, recipientAddress) {
    try {
...
//# sourceMappingURL=meteora-atomic.js.map