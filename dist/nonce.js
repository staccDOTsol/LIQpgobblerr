 ...
 */
import { Connection, Keypair, NonceAccount, NONCE_ACCOUNT_LENGTH, SystemProgram, VersionedTransaction, TransactionMessage, } from '@solana/web3.js';
 ...
 */
export function buildCreateNonceAccountInstructions(payer, nonceKeypair, authority) {
 ...
 */
export async function buildCreateNonceAccountsTransaction(payer, count, authority) {
 ...
 */
export async function getNonceValue(nonceAccountPubkey) {
    try {
 ...
 */
export function buildAdvanceNonceInstruction(noncePubkey, authorizedPubkey) {
 ...
 */
export async function convertToNonceTransaction(transaction, nonceAccountPubkey, nonceAuthority) {
    try {
        // Get the nonce value
 ...
 */
export async function buildNonceTransaction(payer, instructions, nonceAccountPubkey, nonceAuthority) {
    try {
        // Get the nonce value
        const nonceValue = await getNonceValue(nonceAccountPubkey);
        if (!nonceValue) {
 ...
 */
export async function submitWithExponentialBackoff(signedTransaction, maxRetries = 20, initialDelayMs = 500, maxDelayMs = 30000) {
 ...
 */
export async function submitSequentialTransactions(signedTransactions, // base64 encoded
onProgress) {
    const signatures = [];
